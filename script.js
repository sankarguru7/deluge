...
var app = new Vue({
  ...
  data() {
    return {
      ...
      { key: "Unit_Price", label: "Unit Price" }, { key: "tax", label: "Tax" }, { key: "discount", label: "Discount" }, { key: "total_value", label: "Total" }
      ],
      items: [],
      partList: []
    };
  ...
  methods: {
    ...
    qtycalculation(index) {
      ...
      if (app.buttonText == "Approve") {
        ...
        var up = parseFloat(app.items[index].Unit_Price) || 0;
        var dis = parseFloat(app.items[index].Discount) || 0;
        var tax = parseFloat(app.items[index].tax) || 0;
        var linqty = parseFloat(app.items[index].Qty) || 0;
        var total = 0;
        ...
        if (app.selecteddivision == "Cosmetics") {
          var cosdis = parseFloat(dis) / 100 || 0;
          total = (linqty * up);
    ...
    alllinesum() {
      var totalsum = 0;
      app.items.forEach(element => {
        totalsum = totalsum + (parseFloat(element.Total) || 0);
      });
      app.Sum_Total = totalsum.toFixed(2);
    },
